from BudgetApp.src.expense_tracker.transaction import Transaction
...
def main():
    ...
    TRANSACTION_FILE = BASE_DIR / "TRANSACTION_FILE.csv"
...
def get_user_input(transaction_file_path):
    while True:
        ...
        elif expense_type.strip().lower() == "end":
            break
        else:
            print("Invalid input. Please enter 'file' or 'transaction'.")


def get_single_transaction():
    while True:
        transaction_date = input(
            "When was this transaction executed? Please provide a date in format YYYY-MM-DD: "
        )
    ...
    while True:
        transaction_amount = input("Provide the amount of the transaction: ")
        try:
    ...
    while True:
        ...
        if selected_idx in range(len(Transaction.TRANSACTION_CATEGORIES)):
            return Transaction(
                ...
                description=Transaction.TRANSACTION_CATEGORIES[selected_idx],
    ...
    while True:
        ...
        if file_path.exists():
            ...
            with open(file_path, "r") as file:
                ...
                date_col = next(
                    col for col in reader.fieldnames if "date" in col.lower()
                )
                desc_col = next(
                    col for col in reader.fieldnames if "description" in col.lower()
                )
                amount_col = next(
                    col for col in reader.fieldnames if "amount" in col.lower()
                )
                for row in reader:
                    ...
                    transaction = Transaction(
                        ...
                        amount=float(str(row[amount_col]).strip()),
                    )
...
def store_input(transactions, transaction_file_path):
    ...
    with open(transaction_file_path, "a") as file:
        for transaction in transactions:
            file.write(
                f"{transaction.date},{transaction.category},{transaction.description},{transaction.amount}\n"
            )


def summarize_expenses(transaction_file_path):
    df = pd.read_csv(
    ...
    category_totals = (
        df_expenses.groupby("category")["abs_amount"].sum().sort_values(ascending=False)
    )
