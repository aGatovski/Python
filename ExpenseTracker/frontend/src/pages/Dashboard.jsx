...
import { getTransactions, getSummary, addTransactions } from "../api";
...
export default function Dashboard() {
  ...
  async function handleImport() {
    ...
    try {
      const res = await addTransactions(selectedFile);
      const { message } = res.data;
      setImportStatus({ ok: true, msg: message });
      setSelectedFile(null);
      // Refresh data after import
      getTransactions({ month: currentMonth }).then(r => setTransactions(r.data));
      getSummary(currentMonth)
        .then(r => setSummary(r.data))
        .catch(() => setSummary(null));
    } catch (err) {
      const detail = err.response?.data?.detail ?? "Import failed";
      setImportStatus({ ok: false, msg: detail });
    }
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        <div style={{ background: 'white', borderRadius: '8px', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', padding: '1rem' }}>
          ...
          <p style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: 0 }}>
            {summary ? `${summary.net.toFixed(2)} EUR` : "—"}
          </p>
        </div>
        <div style={{ background: 'white', borderRadius: '8px', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', padding: '1rem' }}>
          <p style={{ color: '#6b7280', margin: 0 }}>Total Income</p>
          <p style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#16a34a', margin: 0 }}>
            {summary ? `${summary.income.toFixed(2)} EUR` : "—"}
          </p>
        </div>
        <div style={{ background: 'white', borderRadius: '8px', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', padding: '1rem' }}>
          <p style={{ color: '#6b7280', margin: 0 }}>Total Expenses</p>
          <p style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#dc2626', margin: 0 }}>
            {summary ? `${summary.expenses.toFixed(2)} EUR` : "—"}
          </p>
        </div>
      </div>

      {/* CSV Import */}
      <div style={{ background: 'white', borderRadius: '8px', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', padding: '1rem', marginBottom: '1.5rem' }}>
        <h2 style={{ fontSize: '1.1rem', fontWeight: '600', marginBottom: '0.75rem' }}>Import CSV</h2>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <input
            type="file"
            accept=".csv"
            onChange={e => { setSelectedFile(e.target.files[0] ?? null); setImportStatus(null); }}
            style={{ flex: 1 }}
          />
          <button
            onClick={handleImport}
            disabled={!selectedFile}
            style={{ padding: '0.25rem 0.75rem', cursor: selectedFile ? 'pointer' : 'not-allowed', opacity: selectedFile ? 1 : 0.5 }}
          >
            Import
          </button>
        </div>
        {importStatus && (
          <p style={{ color: importStatus.ok ? '#16a34a' : '#dc2626', margin: '0.5rem 0 0' }}>
            {importStatus.msg}
          </p>
        )}
      </div>
